...
def clip_fitness(individual, img_width, img_height):
    ...
    #return (float(result[0].float().cpu()) * -1) / 10+ (float(result[1].float().cpu()) * -1) *10 + (float(result[2].float().cpu()) * -1)*10,
...
def eaSimple(population, toolbox, cxpb, mutpb, ngen, stats=None, halloffame=None, verbose=__debug__, experiment_name=None, save_folder=None, sub_folder=None, checkpoint=None, render_image=None):
    ...
    if checkpoint:
        ...
        for ind, eval_result in zip(invalid_ind, eval_results):
            ...
            big_sleep.save_individual(latent_space, -1, ind_index)
            ind_index += 1
    ...
    while halloffame[0].fitness.values < TARGET_FITNESS and gen < ngen:
        ...
        for ind, eval_result in zip(invalid_ind, eval_results):
            latent_space = big_sleep.Pars(np.array(ind), all_different=ALL_DIFFERENT, batch_size=BATCH_SIZE)
            big_sleep.save_individual(latent_space, gen, ind_index)
            ind_index += 1
            ind.fitness.values = eval_result
        ...
        if halloffame is not None:
            ...
            #latent_space = big_sleep.Pars(np.array(halloffame[0]))
            #print(latent_space())
            #input()
            #big_sleep.save_individual(latent_space, gen)
            # img_array = chunks(halloffame[0])
            # img = render_image(img_array, size=IMG_HEIGHT)
...
# @profile
def main():
    global POP_SIZE, IMG_SIZE, IMG_WIDTH, IMG_HEIGHT, N_GENS, RANDOM_SEED, CHECKPOINT, CHECKPOINT_FREQ, SAVE_ALL, TOURNAMENT_SIZE, CXPB, MUTPB, MUT_MU, MUT_SIGMA, MUT_INDPB, NUM_LINES, NUM_COLS
    ...
    if CHECKPOINT:
        ...
        experiment_name = f"pylinhas_clip_L{LATENT_SPACE_SIZE}_{RANDOM_SEED if RANDOM_SEED else datetime.now().strftime('%Y-%m-%d_%H-%M')}"
