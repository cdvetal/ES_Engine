...
frase = '''a painting of Cristiano Ronaldo in the Style of Van Gogh'''
...
def save_individual(latent_space, it, ind, pre_scaled=True):
  global frase
  global seed
  path_to_folder = frase
 # if it == 0: os.mkdir(path_to_folder)
  ...
  for img in al:
    ...
    if not pre_scaled:
      img = scale(img, 48*4, 32*4)
    imageio.imwrite(path_to_folder  +"/"  + frase +"-it="+str(it + 1) + "-ind="+str(ind) + '.png', np.array(img))
  al = model(*latent_space(), 0.4).cpu().detach().numpy() ### check the impact of the threshold
  for img in al:
    img = np.array(img)[:,:,:]
    img = np.transpose(img, (1, 2, 0))
    if not pre_scaled:
      img = scale(img, 48*4, 32*4)
    imageio.imwrite(path_to_folder  +"/"  + frase +"-0.4-it="+str(it + 1) + "-ind="+str(ind) + '.png', np.array(img))
