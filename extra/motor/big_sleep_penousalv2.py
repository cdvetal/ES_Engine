...
frase = '''a painting of heaven by van gogh'''
        ...
        for i, layer in enumerate(self.layers):
            if isinstance(layer, GenBlock):
                if (i<17):
                    ...
                    z = layer(z, cond_vector[16].unsqueeze(0), truncation)
        ...
        if ind_numpy is None:
          if CUDA_AVAILABLE:
            ...
            self.thrsh_cls = torch.tensor(1.9).cuda()
              ...
              self.normu = torch.nn.Parameter(torch.zeros(16, 128).normal_(std=1))
              params_other = torch.zeros(16, 1000).normal_(-3.9, .3)
              self.cls = torch.nn.Parameter(params_other)
...
def inicializa():
    ...
    with torch.no_grad():
        al = (model(*lats(), 1).cpu()).numpy() # auqai entram 16 para o modelo
       # for allls in al:
       #      displ(allls)
       #     print('\n')
       # for x in lats():
...
#chega aqui só com uma loss, o best é sempre o zero, e pelo que percebo tb vai sempre "gerar"a imagem correspondente as my_lats
# não entendo pq é que o modelo é tratado como uma array
# note-se que o checkin só serve para gerar a imagem e gravar


def checkin(loss, my_lats=None):        
  #print(loss)
  best = torch.topk(loss[2], k=1, largest=False)[1]
  #print("melhor: " + str(best))
  with torch.no_grad():
    if my_lats is None:
...
def evaluate(latent_space):
  ...
  for ch in range(cutn):
    ...
    apper = out[:, :, offsetx:offsetx + size, offsety:offsety + size] ## são 16?
...
def ascend_txt():
  #print("entrei ascend")
  #print(*lats()[0].size()) #16 128
  #print(*lats()[1].size())  #16 1000

  #print(*lats()[0][0].size()) #128
  #print(*lats()[0][0])
  #print(lats()[0][0])
  
  out = model(*lats(), 1)

  ##al = model(*lats(), 1).cpu().detach().numpy()
  ##for allls in al:
  ##  displ(allls, itt+50000)
  ...
  into = nom(((into) + 1) / 2)
  iii = perceptor.encode_image(into) ## isto chama o clip com as cutn images * batch size?

  

  llls = lats()
  
  #print(llls[0].size()) #torch.Size([16, 128])
  #print(llls[1].size()) #torch.Size([16, 1000])
  ...
  cls_l = ((50*torch.topk(llls[1],largest=False,dim=1,k=999)[0])**2).mean() #a class loss é 50*o valor dos
  return [lat_l, cls_l, -100*torch.cosine_similarity(t, iii, dim=-1).mean()] #aqui calcula a média da cosine similarity, iii devem ser várias imagensm acho
...
if __name__ == '__main__':
  ...
  def train(epoch, i):
    ...
    print(str(itt) +","+ str(loss1[0].item()) +","+ str(loss1[1].item()) +","+ str(loss1[2].item())) 
