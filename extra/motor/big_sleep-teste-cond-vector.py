frase = '''a painting of a woman by picasso'''
...
def displ(img, it=0, pre_scaled=True):
  global frase
  global seed
  ...
  imageio.imwrite(frase + "-seed="+str(seed-1)+"-it="+str(it) + '.png', np.array(img))
...
## é este que é chamado
    def forward(self, z, class_label, truncation):
        ...
        #z=torch.tensor(z[0]).repeat(32,1).float().cuda()
        #z.requires_grad_(True)
        #class_label=torch.tensor(class_label[0]).repeat(32,1).float().cuda()
        #class_label.requires_grad_(True)
       ...
       #aqui
        if ind_numpy is None:
          if CUDA_AVAILABLE:
            ...
            #self.normu = torch.nn.Parameter(torch.tensor(self.normu[0]).repeat(32,1).float().cuda())
            #self.cls = torch.nn.Parameter(torch.tensor(self.cls[0]).repeat(32,1).float().cuda())


            self.thrsh_lat = torch.tensor(1).cuda()
            self.thrsh_cls = torch.tensor(1.9).cuda()
          else:
              print("Isto vai levar com 16")
              self.normu = torch.nn.Parameter(torch.zeros(32, 128).normal_(std=1))
              params_other = torch.zeros(32, 1000).normal_(-3.9, .3)
              self.cls = torch.nn.Parameter(params_other)
        ...
        #   input()
          if all_different:
            if CUDA_AVAILABLE:
              ...
              latent_space_numpy = ind_numpy[ : 128 * batch_size].reshape(batch_size,128)
              imagenet_classes_numpy =  ind_numpy[128 * batch_size : ].reshape(batch_size,1000) 
              self.normu = torch.nn.Parameter(torch.tensor(latent_space_numpy).float())
              self.cls = torch.nn.Parameter(torch.tensor(imagenet_classes_numpy).float())
            ...
            if CUDA_AVAILABLE:
              ...
              latent_space_numpy = ind_numpy[: 128]
              imagenet_classes_numpy =  ind_numpy[128 : ]
              self.normu = torch.nn.Parameter(torch.tensor(latent_space_numpy).repeat(32,1).float())
              self.cls = torch.nn.Parameter(torch.tensor(imagenet_classes_numpy).repeat(32,1).float())
...
def copiado(lats,classes):
    ...
    if CUDA_AVAILABLE:
    #aux_n=lats[0]
        lats=torch.nn.Parameter(torch.tensor(lats[0]).repeat(16,1).float().cuda())
    #aux_c=classes[0]
        classes=torch.nn.Parameter(torch.tensor(classes[0]).repeat(16,1).float().cuda())
    else:
        lats=torch.nn.Parameter(torch.tensor(lats[0]).repeat(16,1).float())
    #aux_c=classes[0]
        classes=torch.nn.Parameter(torch.tensor(classes[0]).repeat(16,1).float())
    return lats,classes
...
def inicializa():
    # aqui temos  a inicialização da seed
    global seed
    global eps
    global lats
    global nom
    ...
    if CUDA_AVAILABLE:
        ...
        t = perceptor.encode_text(tx).detach().clone()
...
def evaluate(latent_space):
  ...
  4*torch.max(torch.square(llls[0]).mean(), latent_space.thrsh_lat)
...
if __name__ == '__main__':
  ...
  def train(epoch, i):
    ...
    if (itt==0):
        print("It,Lat Loss,Class loss,Sim")
    print(str(itt) +","+ str(loss1[0].item()) +","+ str(loss1[1].item()) +","+ str(loss1[2].item()*-1)) 
    #print('Loss')
    ...
    loss.backward()
    # for index, param in enumerate(lats.parameters()):
    #   #print(param.shape)
    #   param.grad[1:,:] = param.grad[0,:]
    #   #print(param.grad)
    #print('Aplicar Adam')
    optimizer.step()
  ...
  with torch.no_grad():
    these = tuple((lats()[0][:1].expand(16, -1), lats()[1][:1].expand(16, -1)))
  ...
  with torch.no_grad():
    ...
    print("torch.mean(lats()[0]) "+str(torch.mean(lats()[0])))
    print("torch.mean(lats()[1]) " + str(torch.mean(lats()[1])))
    print("lats()[0] " + str(lats()[0]))
    print("lats()[1] " + str(lats()[1]))
