...
with tf.device('/CPU'):
    #começa a cena do nima
    base_model = MobileNet((None, None, 3), alpha=1, include_top=False, pooling='avg', weights=None)
    xcenas = Dropout(0)(base_model.output)
    xcenas = Dense(10, activation='softmax')(xcenas)
    model_nima = Model(base_model.input, xcenas)
    model_nima.load_weights('weights/weights_mobilenet_aesthetic_0.07.hdf5')
    #termina a cena do nima
...
frase = '''a painting of superman by van gogh'''
...
def displ(img, it=0, pre_scaled=True, individual=0):
  ...
  imageio.imwrite(frase + "-seed="+str(seed-1)+"-it="+str(it) +"-ind="+str(individual) + '.png', np.array(img))
    ...
    def __init__(self, ind_numpy=None, all_different=False, batch_size=16):
       ...
       #aqui
        if ind_numpy is None:
          if CUDA_AVAILABLE:
            self.normu = torch.nn.Parameter(torch.zeros(16, 256).normal_(std=1).cuda())
            ...
            self.thrsh_cls = torch.tensor(1.9).cuda()
              ...
              self.normu = torch.nn.Parameter(torch.zeros(16, 256).normal_(std=1))
              params_other = torch.zeros(16, 1000).normal_(-3.9, .3)
              self.cls = torch.nn.Parameter(params_other)
...
class CondVectorParameters(torch.nn.Module):
    def __init__(self, ind_numpy, batch_size=16):
        ...
        if CUDA_AVAILABLE:
            self.normu = torch.nn.Parameter(torch.tensor(reshape_array).float().cuda())
            self.thrsh_lat = torch.tensor(1).cuda()
            self.thrsh_cls = torch.tensor(1.9).cuda()
        else:
...
def evaluate(cond_vector_params):
  ...
  if CUDA_AVAILABLE:
    ...
    t = perceptor.encode_text(tx).detach().clone()
  cond_vector = cond_vector_params()
  #input()
  #z = cenas[0]
  #classes = cenas[1]
  #z.data[1 :, :] = z.data[0]
...
if __name__ == '__main__':
  ...
  def train(epoch, i):
    ...
    #     #print(param.grad)
    #     param.grad[2:,:] = torch.zeros((param.shape[0]-2, param.shape[1])).cuda()
    #     #print(param.grad)
    # #input()
    print('Aplicar Adam')
    optimizer.step()
