...
#frase = '''a photograph of a cat'''
#frase = '''a paiting of superman in the style of van gogh'''
    ...
    def __init__(self, ind_numpy=None, all_different=False, batch_size=16):
       ...
       #aqui
        if ind_numpy is None:
          if CUDA_AVAILABLE:
            self.normu = torch.nn.Parameter(torch.zeros(4, 128).normal_(std=1).cuda())
            params_other = torch.zeros(4, 1000).normal_(-3.9, .3)
            self.cls = torch.nn.Parameter(params_other)
            self.thrsh_lat = torch.tensor(1).cuda()
            self.thrsh_cls = torch.tensor(1.9).cuda()
          else:
              self.normu = torch.nn.Parameter(torch.zeros(4, 128).normal_(std=1))
              params_other = torch.zeros(4, 1000).normal_(-3.9, .3)
              self.cls = torch.nn.Parameter(params_other)
...
def evaluate(latent_space):
  ...
  iii = perceptor.encode_image(into) #é aqui que dá merda de memoria
